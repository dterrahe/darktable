...
static void _update_sample_label(dt_lib_module_t *self,
                                 dt_colorpicker_sample_t *sample)
{
  dt_lib_colorpicker_t *data = self->data;
  ...
  switch(data->model)
  {
    ...
    case DT_LIB_COLORPICKER_MODEL_HEX:
      ...
      snprintf(text, sizeof(text), "0x%02X%02X%02X",
               CLAMP(sample->label_rgb[0], 0, 255),
               CLAMP(sample->label_rgb[1], 0, 255),
               CLAMP(sample->label_rgb[2], 0, 255));
      break;
...
static void _update_size(dt_lib_module_t *self,
                         const dt_lib_colorpicker_size_t size)
{
  dt_lib_colorpicker_t *data = self->data;
...
static void _setup_sample(dt_lib_module_t *self,
...
static void _statistic_changed(GtkWidget *widget,
                               dt_lib_module_t *self)
{
  dt_lib_colorpicker_t *data = self->data;
  data->statistic = dt_bauhaus_combobox_get(widget);
  darktable.lib->proxy.colorpicker.statistic = (int)data->statistic;
  dt_conf_set_string("ui_last/colorpicker_mode",
                     dt_lib_colorpicker_statistic_names[data->statistic]);
                                ...
                                dt_lib_module_t *self)
{
  ...
  dt_conf_set_string("ui_last/colorpicker_model",
                     dt_lib_colorpicker_model_names[data->model]);
...
void gui_init(dt_lib_module_t *self)
{
  ...
  dt_lib_colorpicker_t *data = self->data = calloc(1, sizeof(dt_lib_colorpicker_t));
  ...
  gtk_widget_set_no_show_all(color_patch_wrapper,
                             dt_conf_get_bool("ui_last/colorpicker_large") == FALSE);

  // The picker button, mode and statistic combo boxes
  data->statistic_selector =
    dt_bauhaus_combobox_new_full(DT_ACTION(self), NULL, N_("statistic"),
  ...
  data->color_mode_selector =
    dt_bauhaus_combobox_new_full(DT_ACTION(self), NULL, N_("color mode"),
  ...
  GtkWidget *picker_row = dt_gui_hbox(dt_gui_expand(data->statistic_selector),
                                      dt_gui_expand(data->color_mode_selector));
  ...
  gtk_widget_set_name(sample_patch_wrapper, "live-sample");
  ...
  dt_gui_button_ellipsize(data->display_samples_check_box, PANGO_ELLIPSIZE_MIDDLE);
  ...
  dt_gui_button_ellipsize(restrict_button, PANGO_ELLIPSIZE_MIDDLE);
  ...
  // Setting up the GUI
  self->widget = dt_gui_vbox(color_patch_wrapper,
                             picker_row,
                             sample_row_events,
                             dt_ui_section_label_new(C_("section", "live samples")),
                             dt_ui_resize_wrap(data->samples_container, 1,
