...
static inline float pixels2print(dt_lib_export_t *self,
                                 const uint32_t pix)
{
  const dt_dimensions_type_t d_type =
    (dt_dimensions_type_t)dt_bauhaus_combobox_get(self->dimensions_type);
...
static void _export_button_clicked(GtkWidget *widget, dt_lib_export_t *d)
{
  ...
  const dt_colorspaces_color_profile_type_t icc_type =
    dt_conf_get_int(CONFIG_PREFIX "icctype");
...
static void _size_in_px_update(dt_lib_export_t *d)
{
  ...
  else
  {
    ...
    snprintf(size_in_px_txt, sizeof(size_in_px_txt) / sizeof(size_in_px_txt[0]),
             _("which is equal to %s × %s px"),
             gtk_entry_get_text(GTK_ENTRY(d->width)),
...
void gui_reset(dt_lib_module_t *self)
{
  ...
  gtk_entry_set_text(GTK_ENTRY(d->height),
                     dt_confgen_get(CONFIG_PREFIX "height", DT_DEFAULT));
  ...
  dt_bauhaus_combobox_set(d->upscale,
                          dt_confgen_get_bool(CONFIG_PREFIX "upscale",
                                              DT_DEFAULT) ? 1 : 0);
  dt_bauhaus_combobox_set(d->high_quality,
                          dt_confgen_get_bool(CONFIG_PREFIX "high_quality_processing",
                                              DT_DEFAULT) ? 1 : 0);
  dt_bauhaus_combobox_set(d->export_masks,
                          dt_confgen_get_bool(CONFIG_PREFIX "export_masks",
                                              DT_DEFAULT) ? 1 : 0);

  dt_bauhaus_combobox_set(d->intent,
                          dt_confgen_get_int(CONFIG_PREFIX "iccintent",
                                             DT_DEFAULT) + 1);
...
static void _validate_dimensions(dt_lib_export_t *d)
{
  ...
  if(width > d->max_allowed_width || height > d->max_allowed_height)
  {
    width  = width  > d->max_allowed_width
      ? dt_conf_get_int(CONFIG_PREFIX "width")
      : width;
                                     ...
                                     dt_lib_export_t *d)
{
  ...
  if(d_type == DT_DIMENSIONS_CM || d_type == DT_DIMENSIONS_INCH)
  {
    ...
    dt_conf_set_int("metadata/resolution",
                    dt_conf_get_int(CONFIG_PREFIX "print_dpi"));
    _resync_print_dimensions(d);
  }
  else
  {
...
static void _on_storage_list_changed(gpointer instance,
                                     dt_lib_module_t *self)
{
  dt_lib_export_t *d = self->data;
                                                ...
                                                dt_lib_module_t *self)
{
  ...
  {
    const dt_style_t *style = st_iter->data;
...
void gui_init(dt_lib_module_t *self)
{
  ...
  d->print_width = dt_action_entry_new(DT_ACTION(self),
                                       N_("print width"),
                                       G_CALLBACK(_print_width_changed), d,
  ...
  d->print_dpi = dt_action_entry_new(DT_ACTION(self), N_("dpi"),
                                     G_CALLBACK(_print_dpi_changed), d,
                                     _("resolution in dot per inch"),
                                     dt_conf_get_string_const(CONFIG_PREFIX "print_dpi"));

  d->width = dt_action_entry_new(DT_ACTION(self), N_("width"),
                                 G_CALLBACK(_width_changed), d,
                                 _("maximum output width limit.\n"
                                   "click middle mouse button to reset to 0."),
                                 NULL);

  d->height = dt_action_entry_new(DT_ACTION(self), N_("height"),
  ...
  DT_BAUHAUS_COMBOBOX_NEW_FULL
    (d->upscale, self, NULL,
     ...
     dt_conf_get_bool(CONFIG_PREFIX "upscale") ? 1 : 0,
     _callback_bool,
...
void init_presets(dt_lib_module_t *self)
{
  ...
  DT_DEBUG_SQLITE3_PREPARE_V2(
      ...
      " FROM data.presets"
      " WHERE operation='export'", -1, &stmt, NULL);
                    ...
                    size_t *new_size)
{
  if(old_version == 1)
  {
    ...
    memcpy((uint8_t *)new_params + first_half + sizeof(int32_t),
           &sversion, sizeof(int32_t));
    // copy the rest of the old params over
    memcpy((uint8_t *)new_params + first_half + sizeof(int32_t) * 2,
           buf, old_params_size - first_half);
  ...
  else if(old_version == 2)
  {
    ...
    memcpy((uint8_t *)new_params + sizeof(int32_t) * 3,
           (uint8_t *)old_params + sizeof(int32_t) * 2,
           old_params_size - sizeof(int32_t) * 2);
  ...
  else if(old_version == 4)
  {
    ...
    pos += 4 * sizeof(int32_t);
    memcpy((uint8_t *)new_params + pos,
           (uint8_t *)old_params + pos - sizeof(int32_t),
  ...
  else if(old_version == 5)
  {
    ...
    gchar *flags = g_strdup_printf
      ("%x", dt_lib_export_metadata_default_flags() | (omit ? DT_META_OMIT_HIERARCHY : 0));
    ...
    pos += flags_size;
    memcpy((uint8_t *)new_params + pos,
           (uint8_t *)old_params + pos - flags_size,
